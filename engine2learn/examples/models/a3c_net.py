...
class A3CPolicyNetwork(Model):
    ...
    def construct(self):
        ...
        cam, health_and_orient, axis_mappings, action_mappings, advantage, _R, episode_orig_return, len_episode, goals_reached,\
            distance_to_goal, heatmap_image =\
            self.add_feeds(
                        # pure inputs (for train and infer)
                        ([None, self.cam_height, self.cam_width], "cam"),  # the input cam signal
                        ([None, 2], "health_orient"),  # the input health and orientation signal

                        # used to calculate loss of policy function
                        # - the action that got selected (move (-1 to 1), turn (-1 to 1), jump (0 or 1)) in a previous(!) run through the network
                        ([None, self.num_axis_mappings], "axis_mappings"),
                        ([None, self.num_action_mappings], "action_mappings"),
                        # - the calculated advantage (fed in separately, b/c we don't want the V-term within advantage to change the policy weights)
                        ([None], "advantage"),

                        # used to calculate loss of the value function
                        # - the target for our value function (true discounted return measured by monte-carlo roll outs)
                        ([None], "R"),
                        # for Summary only
                        ([], "episode_orig_return"),  # single return (non-normalized/non-discounted) of one episode
                        ([], "len_episode"),  # num steps in the episode
                        ([], "goals_reached"),  # number of times altogether that we reached the goal state
                        ([], "distance_to_goal"),  # the distance in fields to the goal at the end of an episode
                        ([self.cam_height, self.cam_width], "heatmap_image"),  # the heatmap showing already visited positions on the map as red spots
            )

        # convolutional module for processing (and flattening) the cam signal
        cam_expanded = tf.expand_dims(cam, axis=3)
        #conv2d = snt_nets.ConvNet2D(output_channels=[2, 4],  # keep it simple
        #                            kernel_shapes=[(6, 6), (4, 4)],
        #                            strides=[(2, 2), (2, 2)],
        #                            paddings=[snt.SAME],
        #                            activation=tf.nn.relu,
        #                            activate_final=True,
        #                            name="conv2d_module")
        conv2d = snt_nets.ConvNet2D(output_channels=[3],  # keep it simple
                                    kernel_shapes=[(7, 7)],
                                    strides=[(2, 2)],
                                    paddings=[snt.SAME],
                                    activation=tf.nn.relu,
                                    activate_final=True,
                                    name="conv2d_module")
        conv2d_out = conv2d(cam_expanded)  # push cam through our conv layer
