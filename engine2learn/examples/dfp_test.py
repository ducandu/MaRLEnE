...
def main_fun(args, ctx):
    ...
    grid_world = normalize(GridWorldComplex("16x16", save=False, reward_func="rich_potential_jump"), keys_to_globally_normalize=["camera"])
    ...
    with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:{}".format(ctx.task_index), cluster=cluster_spec)):
        ...
        cam_shape = grid_world.observation_space["camera"].shape
        policy_network = A3CPolicyNetwork("policy_network", cam_shape[0], cam_shape[1], hidden_units, num_action_mappings=1, num_axis_mappings=2, beta=beta)
    ...
    with supervisor.managed_session(server.target) as sess:
        ...
        while not supervisor.should_stop() and episode < num_episodes:
            ...
            if args.render:
                grid_world.render()
            observation_dict = grid_world.reset(randomize=True)
            print("STARTING EPISODE {} (reset env):".format(episode))
            print("after reset: _done={} _reward={} pos={} ({},{}) orient={}".format(observation_dict["_done"], observation_dict["_reward"], grid_world._wrapped_env.pos,
                                                                                     grid_world._wrapped_env.x, grid_world._wrapped_env.y, observation_dict["orientation"]))
